...
sink("quad_br_binom.txt") 
cat("
    model{
    
    ## Priors
    c ~ dunif(0, 0.01) # Uniform prior since scale is different from prior biting rates.
    Tmin ~ dnorm(9.4, 1/4^2) # Based on fitted values from Shocket et. al
    Tmax ~ dnorm(39.6, 1/2^2) # Based on fitted values from Shocket et. al
...
jag.data
##### Run JAGS
suffolk.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, model.file="quad_br_binom.txt",
...
##### Run JAGS
albany.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, model.file="quad_br_binom.txt",
                    n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni, DIC=TRUE, working.directory=getwd())
albany.out 
mcmcplot(albany.out)
...
saveRDS(albany.out, file="jagsout_a_albany.RDS")
saveRDS(suffolk.out, file="jagsout_a_suffolk.RDS")
