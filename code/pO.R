...
temp <- seq(0, 50, 0.1)
plot(temp, quad_model(temp, 8.2, 33.2, 0.00445), type="l")
...
cat("
    ...
    Tmin ~ dnorm(8.2, 1/3^2) # 95% likely in interval (2.2, 14.2)
    Tmax ~ dnorm(33.2, 1/3^2 ) # 95% likely in interval (27.2, 39.2)
...
inits<-function(){list(
  Tmin = runif(1, min=4, max=12),
  Tmax = runif(1, min=30, max=35),
...
suffolk.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, 
                    model.file="quad_pO_binom.txt", n.thin=nt, n.chains=nc, 
...
plot(data.suffolk$temperature, data.suffolk$proportion, xlab="Temperature [°C]",
     ylab="pO", xlim=c(0, 50), ylim=c(0, 1), pch=20, col=suffolk.col, 
     main="Suffolk (binomial)")
...
plot(data.albany$temperature, data.albany$proportion, xlab="Temperature [°C]",
     ylab="pO", xlim=c(0, 50), ylim=c(0, 1), pch=20, col=albany.col, 
     main="Albany (binom)")
...
saveRDS(albany.out, file="jagsout_pO_albany.RDS")
saveRDS(suffolk.out, file="jagsout_pO_suffolk.RDS")
