...
cat("
    ...
    Tmin ~ dnorm(8.2, 1/(3*0.5)^2) 
    Tmax ~ dnorm(33.2, 1/(3*0.5)^2 ) 
...
suffolk.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, 
                    model.file="quad_pO_binom_sp.txt", n.thin=nt, n.chains=nc, 
...
saveRDS(albany.out, file="jagsout_pO_albany_sp.RDS")
saveRDS(suffolk.out, file="jagsout_pO_suffolk_sp.RDS")
