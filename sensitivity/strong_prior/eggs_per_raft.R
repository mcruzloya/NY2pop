...
sink("quad_inf_epr_sp.txt")
cat("
    model{
    
    ## Priors
    Tmin ~ dnorm(5.3, 1 / (0.5*2.5)^2) 
    Tmax ~ dnorm(38.9, 1 / (0.5*2.5)^2) 
...
suffolk.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, 
                    model.file="quad_inf_epr_sp.txt", n.thin=nt, n.chains=nc, 
...
saveRDS(albany.out, file="jagsout_epr_albany_sp.RDS")
saveRDS(suffolk.out, file="jagsout_epr_suffolk_sp.RDS")
