...
cat("
    ...
    Tmin ~ dnorm(7.8, 1/(0.5*2)^2) 
    Tmax ~ dnorm(38.4, 1/(0.5*2)^2 )
...
suffolk.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, 
                    model.file="quad_pLA_binom_sp.txt", n.thin=nt, n.chains=nc, 
...
saveRDS(albany.out, file="jagsout_pLA_albany_sp.RDS")
saveRDS(suffolk.out, file="jagsout_pLA_suffolk_sp.RDS")
