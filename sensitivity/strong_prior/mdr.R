...
data.suffolk
plot(data.suffolk$temperature, 1 / data.suffolk$days.to.emergence, 
     xlim=c(10, 35), ylim=c(0, 0.2),
     pch=20, main="Suffolk")
plot(data.albany$temperature, 1 / data.albany$days.to.emergence, xlim=c(10, 35), ylim=c(0, 0.2),
     pch=20, main="Albany")
...
# Visualize data.
g <- (ggplot(data, aes(temperature, days.to.emergence)) +  
        geom_violin(aes(col=population, group=cut_width(temperature, 1))) + 
        geom_point(aes(col=population, alpha=I(0.3))) +
        facet_wrap(~population, nrow=1) +
        theme_classic() + scale_color_manual(values=c(albany.col, suffolk.col)))
...
cat("
    ...
    Tmin ~ dnorm(5, 1/(2.5 * 0.5)^2)
    Tmax ~ dnorm(38.5, 1/(1.5 * 0.5)^2) 
...
jag.data <- list(trait = trait, N.obs = N.obs, temp = temp)
##### Run JAGS
albany.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, model.file="briere_mdr_sp.txt",
...
points(data.albany$temperature, 1 / data.albany$days.to.emergence, pch=20,
       col=alpha(albany.col, 0.3) )
...
data.mean.suf <- c(mean(1 / data.suffolk$days.to.emergence[data.suffolk$temperature == 22]),
                   mean(1 / data.suffolk$days.to.emergence[data.suffolk$temperature == 25]),
                   mean(1 / data.suffolk$days.to.emergence[data.suffolk$temperature == 28]))
data.sd.suf <- c(sd(1 / data.suffolk$days.to.emergence[data.suffolk$temperature == 22]),
                 sd(1 / data.suffolk$days.to.emergence[data.suffolk$temperature == 25]),
                 sd(1 / data.suffolk$days.to.emergence[data.suffolk$temperature == 28]))
data.sderr.suf <- data.sd.suf / sqrt(c(length(days.to.emergence$temperature == 22),
                                       length(days.to.emergence$temperature == 25),
                                       length(days.to.emergence$temperature == 28)))
...
data.mean.alb <- c(mean(1 / data.albany$days.to.emergence[data.albany$temperature == 22]),
                   mean(1 / data.albany$days.to.emergence[data.albany$temperature == 25]),
                   mean(1 / data.albany$days.to.emergence[data.albany$temperature == 28]))
data.sd.alb <- c(sd(1 / data.albany$days.to.emergence[data.albany$temperature == 22]),
                 sd(1 / data.albany$days.to.emergence[data.albany$temperature == 25]),
                 sd(1 / data.albany$days.to.emergence[data.albany$temperature == 28]))
...
saveRDS(albany.out, file="jagsout_MDR_albany_sp.RDS")
saveRDS(suffolk.out, file="jagsout_MDR_suffolk_sp.RDS")
