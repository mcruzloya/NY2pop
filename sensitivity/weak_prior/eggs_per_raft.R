...
sink("quad_inf_epr_wp.txt")
cat("
    model{
    
    ## Priors
    Tmin ~ dnorm(5.3, 1 / (2*2.5)^2) #  Shocket et al. estimate with inflated std
    Tmax ~ dnorm(38.9, 1 / (2*2.5)^2) # Shocket. et al. estimate with inflated std
...
suffolk.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, 
                    model.file="quad_inf_epr_wp.txt", n.thin=nt, n.chains=nc, 
...
saveRDS(albany.out, file="jagsout_epr_albany_wp.RDS")
saveRDS(suffolk.out, file="jagsout_epr_suffolk_wp.RDS")
