...
cat("
    ...
    Tmin ~ dnorm(7.8, 1/(2*2)^2) 
    Tmax ~ dnorm(38.4, 1/(2*2)^2 )
...
suffolk.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, 
                    model.file="quad_pLA_binom_wp.txt", n.thin=nt, n.chains=nc, 
...
saveRDS(albany.out, file="jagsout_pLA_albany_wp.RDS")
saveRDS(suffolk.out, file="jagsout_pLA_suffolk_wp.RDS")
