...
cat("
    ...
    Tmin ~ dnorm(5, 1/(2.5 * 2)^2)
    Tmax ~ dnorm(38.5, 1/(1.5 * 2)^2) 
...
jag.data <- list(trait = trait, N.obs = N.obs, temp = temp)
##### Run JAGS
albany.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, model.file="briere_mdr_wp.txt",
...
saveRDS(albany.out, file="jagsout_MDR_albany_wp.RDS")
saveRDS(suffolk.out, file="jagsout_MDR_suffolk_wp.RDS")
